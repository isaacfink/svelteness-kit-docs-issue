...
<script>
    export let id
    import {reexecute} from '$lib/stores/store'
    import { operationStore, query, mutation } from "@urql/svelte";
    import { onMount } from "svelte";

    import { CATEGORY } from "$lib/queries/admin/queries";
    import {UPDATE_CATEGORY, REORDER_PACKAGES, DELETE_CATEGORY} from "$lib/queries/admin/mutations";
    import {goto} from '$app/navigation'
    import DragDrop from "$lib/components/admin/categories/DragDropPackage.svelte";
    ...
    const reorderPackages = mutation({query:REORDER_PACKAGES})
    ...
    function saveChanges(){
      ...
      reorderPackages({order:tempOrder}).then(res => {
        ...
        saveMutation(mutationOptions).then((res) => {
          if (res?.error?.message == "[GraphQL] Unauthenticated!") {
                goto('/accounts/login?next=/admin/categories/new')
            }
          $category.reexecute()
    ...
    category.subscribe(res => {
      let tempList = []
      res.data?.category?.packageSet.edges?.forEach(c => {
        tempList.push({
    ...
    function deletePackage(){
      console.log('deleting package');
        deleteMutation({ids:[$category.data.category.id]}).then(res => {
          if (res?.error?.message == "[GraphQL] Unauthenticated!") {
                goto('/accounts/login?next=/admin/categories/new')
            }

          if (res.error || res.data.deleteCategory.deletedIds.length < 1) {
...
<form on:submit|preventDefault="{saveChanges}" class="space-y-8 divide-y divide-gray-200 pb-5">
    ...
    <div>
      ...
      <div class="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
        ...
        <div class="sm:col-span-3">
          ...
          <div class="mt-1 flex items-center">
            <span class="h-12 w-12 overflow-hidden">
              <img src="http://127.0.0.1:8000/{$category.data.category.image}" alt="">
            </span>
            <input bind:files={image} id="image" type="file" class="ml-5 bg-white py-2 px-3 border border-gray-300 rounded-md shadow-sm text-sm leading-4 font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-primary">
          </div>
        </div>
        ...
        <div class="sm:col-span-3">
          ...
          <div class="mt-1 flex items-center">
            <span class="h-12 w-12 overflow-hidden">
              <img src="http://127.0.0.1:8000/{$category.data.category.icon}" alt="">
            </span>
        ...
        <div class="sm:col-span-3">
          ...
          <p>{$category.data.category.type}</p>
          <p>{type}</p>
          <div class="mt-1">
            <select id="type" name="type" bind:value="{type}" autocomplete="none" class="shadow-sm focus:ring-purple-primary focus:border-purple-primary block w-full sm:text-sm border-gray-300 rounded-md">
      ...
      <div class="h-6"></div>
      <p class="mb-2">{$category.data.category.packageSet.edges.length} packages</p>
  ...
  <div class="pt-5">
    <div class="flex justify-end">
      ...
      <button on:click|preventDefault={toggleConfirmation} type="button" class="ml-3 bg-white py-2 px-4 border border-red-500 rounded-md shadow-sm text-sm font-medium text-red-500 hover:bg-red-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring:red-500">Delete</button>
  ...
  <div on:click|self={toggleConfirmation} class="inset-0 sticky h-screen top-0 bg-gray-700 bg-opacity-75 z-infinite flex items-center justify-center">
    <div class="relative inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
      ...
      <div class="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
        <button on:click|preventDefault={deletePackage} type="button" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:ml-3 sm:w-auto sm:text-sm">Delete</button>
        <button on:click|preventDefault={toggleConfirmation} type="button" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:w-auto sm:text-sm">Cancel</button>
      </div>
    </div>
  </div>
