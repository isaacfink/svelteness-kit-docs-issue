<script>
    import { activePackage, selectedItem, selectedProduct } from '$lib/stores/store';
    
        export let product
        import {increaseOption as increase, decreaseOption as decrease} from '$lib/utilties/cartUtils'
        import { operationStore } from '@urql/svelte';
        $: isSelected = amount > 0
        let amount = 0
    
        activePackage.subscribe((current) => {
            amount = current.items?.find(i => i.id == $selectedItem?.id)?.products?.find(p => p.id == $selectedProduct.id)?.options?.find(o => o.id == product.id)?.quantity || 0
        })
