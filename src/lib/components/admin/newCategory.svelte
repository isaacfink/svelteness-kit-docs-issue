<script>
    ...
    import {ALL_SCHEDULES_LIMITED, UPDATE_CATEGORY} from "$lib/queries/admin";
...
<form on:submit|preventDefault="{saveChanges}" class="space-y-8 divide-y divide-gray-200 pb-5">
    ...
    <div>
        ...
        <div class="sm:col-span-3">
            ...
            <div class="mt-1">
            <select bind:this="{type}" id="type" name="type" autocomplete="none" class="shadow-sm focus:ring-purple-primary focus:border-purple-primary block w-full sm:text-sm border-gray-300 rounded-md">
                <option value="1" selected={category.type == 'A_1'}>Regular</option>
                <option value="2" selected={category.type == 'A_2'}>Minimum amount per order</option>
                <option value="3" selected={category.type == 'A_3'}>Prefilled</option>
            </select>
            </div>
        </div>
