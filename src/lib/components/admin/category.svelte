<script>
    ...
    import { UPDATE_CATEGORY, ALL_SCHEDULES_LIMITED, DELETE_CATEGORY, DELETE_PACKAGE, REORDER_PACKAGES_MUTATION} from "$lib/queries/admin";
    ...
    let schedules = operationStore(ALL_SCHEDULES_LIMITED)
    ...
    <div>
      ...
      <div class="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
        ...
        <div class="sm:col-span-3">
          ...
          <div class="mt-1">
            <select id="type" name="type" autocomplete="none" class="shadow-sm focus:ring-purple-primary focus:border-purple-primary block w-full sm:text-sm border-gray-300 rounded-md">
              <option selected={$category.data.category.type == 'A_1'}>Regular</option>
              <option selected={$category.data.category.type == 'A_2'}>Minimum amount per order</option>
              <option selected={$category.data.category.type == 'A_3'}>Prefilled</option>
            </select>
          </div>
        </div>
          ...
          <div class="mt-1">
            <select bind:this={schedule} id="schedule" name="schedule" autocomplete="none" class="shadow-sm focus:ring-purple-primary focus:border-purple-primary block w-full sm:text-sm border-gray-300 rounded-md">
              {#each $schedules?.data?.schedules || [] as schedule}
                <option selected={$category.data.category?.schedule?.id == schedule.id} value="{schedule.id}">{schedule.name}</option>
              {/each}
            </select>
          </div>
        </div>
      ...
      {:else if selectedMenu == 'packages'}
        <SortableList
        list={$category?.data?.category.packageSet} 
        key="id" 
        on:sort={sortList}
        let:item 
      >
